...
std::map<string, User> user;  // 记录用户信息的map
...
BOOL SpriteCollisin(Sprite* pSpriteHitter, Sprite* pSpriteHittee)
{
        ...
        (pHittee == g_pBombBitmap && (pHitter == g_pLeftSubmarineBitmap || pHitter == g_pRightSubmarineBitmap)))
    {
        ...
        // 更新服务器用户目前得分
        if (user.find(getscorename) != user.end())
        {
            user[getscorename].nowscore = g_iScore;
            saveinfo("./text.txt", user);
        }
        //only kill tomb
        pSpriteHitter->kill();
        pSpriteHittee->kill();
...
void NewGame()
{
    ...
    user.clear();
    inituserinfo("./text.txt", user);
    if (user.find(getscorename) == user.end())
    {
        user.insert((make_pair(getscorename, User(getscorename))));
        saveinfo("./text.txt", user);
    }

    // 更新最高分
    g_MaxScore = user[getscorename].maxscore;

    // 更新血量和上一次的目前得分
    HP = user[getscorename].blood;
    g_iScore = user[getscorename].nowscore;
...
void GamePaint(HDC hDC)
{
    ...
    else
    {
        ...
        if (user.find(getscorename) != user.end())
        {
            user[getscorename].maxscore = g_MaxScore;
            saveinfo("./text.txt", user);
        }
    }
    SetBkMode(hDC, TRANSPARENT);
